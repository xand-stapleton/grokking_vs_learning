...
def main(args):
    ...
    assert (
        args.activation in activation_dict
    ), f"Unsupported activation function: {args.activation}"
    ...
    # create optimizer
    assert (
        args.optimizer in optimizer_dict
    ), f"Unsupported optimizer choice: {args.optimizer}"
    ...
    with tqdm(total=args.optimization_steps, dynamic_ncols=True) as pbar:
        for x, labels in islice(cycle(train_loader), args.optimization_steps):
            ...
            if do_log:
                train_losses.append(
                    compute_loss(
                        mlp, train, args.loss_function, device, N=len(train)
                    )
                )
                ...
                test_losses.append(
                    compute_loss(
                        mlp, test, args.loss_function, device, N=len(test)
                    )
                )
            ...
            if args.filter == "none":
                ...
                grads = gradfilter_ema(
                    mlp, grads=grads, alpha=args.alpha, lamb=args.lamb
                )
            else:
                raise ValueError(
            ...
            #     plt.grid()
            #     plt.savefig(f"../../large_files/plots/mnist_acc_{args.label}.png", dpi=150)
            #     plt.close()
            ...
            #     plt.grid()
            #     plt.savefig(f"../../large_files/plots/mnist_loss_{args.label}.png", dpi=150)
            #     plt.close()
            ...
            torch.save(
                ...
                f"../../large_files/grokfast/mnist_{args.label}_usegrokfast_{args.filter}.pt",
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--download_directory",
        type=str,
        default="/Users/dmitrymanning-coe/Documents/Research/Grokking/ModAddition/large_files",
    )
