...
def model_update(
    ...
    if train_type == "Ising":
        ...
        for batch, (X_train, y_train) in enumerate(train_loader):
            ...
            if loss_criterion == "MSE":
                ...
                train_correct += torch.sum(
                    predicted_labels == y_train.to(device)
                )
                ...
                predicted_labels = torch.argmax(y_pred, dim=1)
                train_correct += (
                    (predicted_labels == y_train.to(device)).sum().item()
                )
                train_loss = loss_function(y_pred, y_train.to(device))
            # Forward pass
        ...
        average_train_loss = (
            train_loss / len(train_loader)
        )  # Note that with no argument passed to a loss function, it will calculate the mean, so you just need to divide by batch number to get total mean
...
def train(
    ...
    for i in tqdm(range(epochs)):
        if compare_models != None:
            # So... it's the same for  both conditions lool.
            if i < compare_models:
        ...
        train_loss, train_accuracy = model_update(
            train_type,
            train_loader,
            criterion,
            optimizer,
        ...
        if i > compare_models:
            cosines_tensor = calculate_cosine_similarity(
                back_compare[0], model, weight_keys=None
            )[0]
            ...
            cosine_steps_tensor = calculate_cosine_similarity(
                back_steps_model, front_steps_model, weight_keys=None
            )[0]
        ...
        if i % 50 == 0 or i == epochs - 1:
            ...
            if train_type == "Ising":
                print(
                    f"epoch: {i}, train loss: {train_loss.item():.4f}, accuracy: {train_accuracy[-1]:.4f}"
                )
                print(
                    12 * " "
                    + f"test loss: {test_loss.item():.4f}, accuracy: {test_accuracy[-1]:.4f}"
                )
                print(60 * "*")
...
def compute_loss_mnist(
    ...
    model.train()  # Set model back to training mode
    return total_loss / count if points > 0 else 0
