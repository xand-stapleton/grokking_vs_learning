...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Example script with --grok and --no-grok flags."
    )
    ...
    config_file = (
        "configs/mod_config_grok.yaml"
        if known_args.grok
        else "configs/mod_config_nogrok.yaml"
    )
    ...
    analysable_quantities = config["analysable_quantities"]
    bs = int(config["bs"])
    ...
    for seed_triple in seeds:
        ...
        fisher_dataset = FisherDataset(test_dataset, fisher_bs)
        ...
        fisher_loader = DataLoader(fisher_dataset, fisher_bs, shuffle=True)

        save_object.train_loader = train_loader
        save_object.test_loader = test_loader
        save_object.fisher_loader = fisher_loader

        network = MLP(weight_multiplier=weight_multiplier, P=P)

        save_name = f"{args.root_dir}/grok_dataseed_{data_seed}_sgdseed_{sgd_seed}_initseed_{init_seed}_wd_{weight_decay}_wm_{weight_multiplier}_time_{int(time.time())}"
        model = Model(
            args,
            optimiser,
            network,
        ...
        one_run_object = model.train(
            ...
            device=device,
            evaluate_dic=analysable_quantities,
            fisher_loader=fisher_loader,
        )

        # Save the train parameters
        with open(f"{save_name}/save_obj.pt", "wb") as dill_file:
            torch.save(save_object, dill_file)

        # Save the one run object
        with open(f"{save_name}/onerun.pt", "wb") as dill_file:
            torch.save(one_run_object, dill_file)
        ...
        for layer in layer_indices:
            ...
            with open(
                f"{save_name}/fim_prune_loss_acc_{layer}-pruned.pt", "wb"
            ) as dill_file:
