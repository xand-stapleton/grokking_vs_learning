...
def main(args, layer):
    ...
    for wm_path_pair in weight_multipliers_and_paths:
        ...
        moving_avg_y = 0.045 * wm_seed_avg_loss / np.min(wm_seed_avg_loss, axis=0)
        moving_avg_y = running_mean(moving_avg_y, N=5)
        ...
        moving_avg_y = 100 * wm_seed_avg_acc / np.max(wm_seed_avg_acc, axis=0)
        moving_avg_y = running_mean(moving_avg_y, N=5)
        acc_plot_data = acc_plot_data | {
            f"{wm}": {
                ...
                * np.arange(len(wm_seed_avg_acc))
                ...
                # Fix the weird scaling problem in the data
                "y": moving_avg_y,
                "color": f"{pick_colours(float(wm), 0.1, 10)}",
            }
        }
...
if __name__ == "__main__":
    ...
    for layer in [None] + list(range(12)):
        main(args, layer)
