...
import numpy as np
import torch
from simple_plot import create_pruning_plot, value_to_rgb
import argparse
from pathlib import Path
...
def main(args):
	...
	for wm_path_pair in weight_multipliers_and_paths:
		...
		if isinstance(wm_seed_avg_acc, float):
			wm_seed_avg_acc = [wm_seed_avg_acc]
		if isinstance(wm_seed_avg_loss, float):
			wm_seed_avg_loss = [wm_seed_avg_loss]

		loss_plot_data = loss_plot_data | {
			f"{wm}": {
				"x": 100
				* np.arange(len(wm_seed_avg_acc))
				/ len(wm_seed_avg_loss),
				"y": wm_seed_avg_loss,
				"color": f"{value_to_rgb(wm, 0.1, 10)}",
			}
		}
		...
		acc_plot_data = acc_plot_data | {
			f"{wm}": {
				...
				* np.arange(len(wm_seed_avg_acc))
				/ len(wm_seed_avg_acc),
				# Fix the weird scaling problem in the data
				"y": 100 * wm_seed_avg_acc / np.max(wm_seed_avg_acc, axis=0),
				"color": f"{value_to_rgb(wm, 0.1, 10)}",
			}
		}
	...
	fig = create_pruning_plot(
		...
		text_sizes={"title": 16, "axis_labels": 18, "legend": 14},
		save_path="global_pruning_loss_curves.pdf",
	)
	fig.show()
	...
	fig = create_pruning_plot(
		...
		text_sizes={"title": 16, "axis_labels": 18, "legend": 14},
		save_path="global_pruning_acc_curves.pdf",
	)
	fig.show()
...
def get_weight_multiplier_paths(parent_dir):
	...
	pattern = re.compile(r"hiddenlayer_\[\d+\]_desc_mod_add_wm_(\d+\.\d+)_.*")
...
def aggregate_seed_metrics_from_files(base_dir):
	"""
	...
	for file_path in Path(base_dir).rglob("fim_prune_loss_acc.pt"):
		loss, acc = torch.load(
			file_path
