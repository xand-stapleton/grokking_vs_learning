...
class CNN(nn.Module):
    ...
    def __init__(
        ...
        # optimizer set at start of training.
...
# CNN that automatically calculates the size of the layers
import torch
import torch.nn as nn
...
class CNN_Rect(nn.Module):
    ...
    def __init__(
        ...
        x = sample_input
        for layer in layers:  # Run through conv layers and flatten
            x = layer(x)
        flattened_size = x.shape[1]  # Size after flattening
        ...
        # Create single Sequential for all layers
        self.model = nn.Sequential(*layers)
